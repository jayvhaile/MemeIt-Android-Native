...
class BitmapToVideoEncoder(private val mCallback: IBitmapToVideoEncoderCallback) {
    ...
    val activeBitmaps: Int
        get() = mEncodeQueue.size
    ...
    fun startEncoding(width: Int, height: Int, outputFile: File) {
        ...
        try {
            outputFileString = outputFile.canonicalPath
        } catch (e: IOException) {
        ...
        GlobalScope.launch(Dispatchers.Main, CoroutineStart.DEFAULT) {
            ...
            mCallback.onEncodingComplete(file)
        }
    }
    ...
    private fun encode(): File? {
        ...
        while (true) {
            ...
            var bitmap: Bitmap? = mEncodeQueue.poll()
            if (bitmap == null) {
                synchronized(mFrameSync) {
            ...
            val encoderStatus = mediaCodec!!.dequeueOutputBuffer(mBufferInfo, timeoutUs)
            ...
            for (i in 0 until width) {
                ...
                if (j % 2 == 0 && index % 2 == 0) {
                    try {
                        ...
                        yuv420sp[uvIndex+1] = (if (V < 0) 0 else if (V > 255) 255 else V).toByte()
                    } catch (e: Exception) {
                    }
    ...
    companion object {
        ...
        private fun selectColorFormat(codecInfo: MediaCodecInfo,
                                      mimeType: String): Int {
